...
def test_health_check():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Services: Git={services.get('git', False)}, Rules={services.get('rules', False)}, Performance={services.get('performance', False)}"
            )
...
def test_backend_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Status endpoint", False, duration, f"Status {response.status_code}"
            )
            return False, None
    except Exception as e:
        duration = time.time() - start
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                log_test(
                    "Chat endpoint", False, duration, data.get("error", "Unknown error")
                )
        else:
...
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Validation endpoint", False, duration, f"Status {response.status_code}"
            )
    except Exception as e:
        duration = time.time() - start
...
def test_stats_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Stats endpoint", False, duration, f"Status {response.status_code}"
            )
    except Exception as e:
        duration = time.time() - start
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "current" in data:
                ...
                print(
                    f"   Memory: {current.get('memory', {}).get('current_mb', 0):.1f} MB"
                )
                if "response_times" in current and current["response_times"]:
                    print(
                        f"   Avg Response Time: {current['response_times'].get('avg_ms', 0):.1f}ms"
                    )
...
def test_git_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Git status endpoint", False, duration, f"Status {response.status_code}"
            )
    except requests.exceptions.Timeout:
        duration = time.time() - start
...
def test_rules_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Rules endpoint", False, duration, f"Status {response.status_code}"
            )
    except requests.exceptions.Timeout:
        duration = time.time() - start
...
def calculate_performance_summary():
    ...
    if results["performance"]:
        ...
        for endpoint, duration in sorted(
            results["performance"].items(), key=lambda x: x[1]
        ):
