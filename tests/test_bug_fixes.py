...
def test_error_context_handling():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        test_cases = [
            ...
            (
                "Error: ValueError\nTraceback...",
                "Valid error should extract first line",
            ),
        ...
        for error_context, description in test_cases:
            try:
                ...
                assert (
                    len(query) > 0 or query == "user_message"
                ), f"{description}: Query should be valid"
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        for provider, model_name, description in test_cases:
            try:
                ...
                ), f"{description}: Provider should not be None"
                assert (
                    model_name is not None
                ), f"{description}: Model name should not be None"
                print(f"  [OK] {description}")
                passed += 1
            ...
            if (
                ...
                if should_pass:
                    raise AssertionError(
                        f"{description}: Should have passed but failed"
                    )
                ...
                if not should_pass:
                    raise AssertionError(
                        f"{description}: Should have failed but passed"
                    )
