...
def test_invalid_requests():
    ...
    test_cases = [
        ...
        (
            "GET /api/files with invalid path",
        ...
        try:
            if "chat" in name:
                response = requests.post(
                    f"{BASE_URL}/api/chat", json=payload, timeout=5
                )
            elif "files" in name:
                response = requests.get(
                    f"{BASE_URL}/api/files", params=payload, timeout=5
                )
            else:
                continue
            ...
            if response.status_code in [200, 400, 422]:
                print(
                    f"  [OK] {description}: Handled gracefully (status {response.status_code})"
                )
                passed += 1
            else:
                print(
                    f"  [WARN] {description}: Unexpected status {response.status_code}"
                )
...
def test_large_payload():
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat", json={"message": large_message}, timeout=30
        )
