...
class CodeInstrumentation:
    ...
    def _find_instrumentation_points(
        ...
        return sorted(
            points, key=lambda x: x.line_number, reverse=True
        )  # Reverse for insertion

    def _insert_instrumentation(
        self, original_content: str, points: List[InstrumentationPoint]
    ) -> str:
    ...
    def _generate_log_statement(self, point: InstrumentationPoint) -> str:
        ...
        if point.type == "function_entry":
            ...
            return (
                f'print(f"[{log_id}] COND {point.context}: {{{point.code_snippet}}}")'
            )
        else:
    ...
    def clean_instrumentation(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            lines = content.split("\n")
            cleaned_lines = [
                line for line in lines if self.instrumentation_id not in line
            ]
