...
class FactsExtractor:
    ...
    def __init__(self):
        ...
        self.file_pattern = re.compile(
            r"(?:created|added|modified|wrote)\s+(?:file|files)?\s*:?\s*([^\s,]+\.\w+)",
            re.IGNORECASE,
        )
        self.function_pattern = re.compile(
            r"(?:added|created|implemented)\s+(?:function|method)\s+(\w+)",
            re.IGNORECASE,
        )
        self.class_pattern = re.compile(
            r"(?:added|created|implemented)\s+class\s+(\w+)", re.IGNORECASE
        )
        self.error_pattern = re.compile(
            r"(?:fixed|resolved|solved)\s+(?:error|bug|issue)\s*:?\s*(.+)",
            re.IGNORECASE,
        )
    ...
    def _extract_from_message(self, content: str) -> List[Fact]:
        ...
        if re.search(r"(?:decided|chose|selected|using)\s+", content, re.IGNORECASE):
            decision_match = re.search(
                r"(?:decided|chose|selected|using)\s+(.+)", content, re.IGNORECASE
            )
            if decision_match:
    ...
    def update_facts(
        self, existing_facts: List[Fact], new_messages: List[Dict[str, str]]
    ) -> List[Fact]:
