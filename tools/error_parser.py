...
class ErrorParser:
    ...
    def __init__(self):
        ...
        self.traceback_pattern = re.compile(
            r'File\s+["\']([^"\']+)["\'],\s+line\s+(\d+),\s+in\s+(\w+)', re.MULTILINE
        )

        # Error type and message pattern
        self.error_pattern = re.compile(
            r"^(\w+(?:Error|Exception|Warning)):\s*(.+)$", re.MULTILINE
        )
    ...
    def is_python_error(self, text: str) -> bool:
        ...
        return bool(
            self.traceback_pattern.search(text) or self.error_pattern.search(text)
        )
