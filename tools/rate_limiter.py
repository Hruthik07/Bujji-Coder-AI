...
class RateLimiter:
    ...
    async def _check_rate_limit(
        ...
        async with _rate_limit_lock:
            ...
            requests[:] = [req_time for req_time in requests if now - req_time < self._get_window_seconds(window)]
            
            # Check if limit exceeded
            if len(requests) >= limit:
                reset_time = int(requests[0] + self._get_window_seconds(window)) if requests else int(now)
    ...
    async def check_rate_limit(
        ...
        # Check all windows
        checks = {
            "minute": await self._check_rate_limit(identifier, "minute", self.requests_per_minute),
            "hour": await self._check_rate_limit(identifier, "hour", self.requests_per_hour),
            "day": await self._check_rate_limit(identifier, "day", self.requests_per_day)
        }
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not is_allowed:
                ...
                raise HTTPException(
                    ...
                    headers={
                        "X-RateLimit-Limit": str(rate_limit_info["limit"].get(exceeded_window, "unknown")),
                        "X-RateLimit-Remaining": "0",
                        "X-RateLimit-Reset": str(rate_limit_info["reset"].get(exceeded_window, int(time.time())))
                    }
                )
