...
class ValidationService:
    ...
    def validate_file(self, file_path: str, content: str) -> ValidationResult:
        ...
        if file_ext == ".py":
            type_check_passed, type_issues = self._validate_types_python(
                full_path, content
            )
            issues.extend(type_issues)
        elif file_ext in [".ts", ".tsx"]:
            type_check_passed, type_issues = self._validate_types_typescript(
                full_path, content
            )
            issues.extend(type_issues)
        ...
        valid = (
            syntax_valid
            and all(issue.severity == ValidationSeverity.ERROR for issue in issues)
            == False
        )
    ...
    def _validate_types_python(
        self, file_path: Path, content: str
    ) -> Tuple[Optional[bool], List[ValidationIssue]]:
        ...
        try:
            result = subprocess.run(
                ["mypy", "--version"], capture_output=True, text=True, timeout=5
            )
    ...
    def _validate_types_typescript(
        self, file_path: Path, content: str
    ) -> Tuple[Optional[bool], List[ValidationIssue]]:
        ...
        try:
            result = subprocess.run(
                ["tsc", "--version"], capture_output=True, text=True, timeout=5
            )
        except (subprocess.TimeoutExpired, FileNotFoundError):
    ...
    def _lint_python(
        ...
        try:
            result = subprocess.run(
                ["flake8", "--version"], capture_output=True, text=True, timeout=5
            )
        except (subprocess.TimeoutExpired, FileNotFoundError):
