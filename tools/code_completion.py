...
class CodeCompletionEngine:
    ...
    def __init__(
        self, rag_system: Optional[RAGSystem] = None, workspace_path: str = "."
    ):
        ...
        try:
            self.llm_provider = (
                get_provider("deepseek", Config.DEEPSEEK_API_KEY)
    ...
    def get_completions(
        ...
        ast_completions = self._get_ast_completions(
            file_path, file_content, context, language
        )
    ...
    def _get_ast_completions(
        self, file_path: str, file_content: str, context: Dict[str, Any], language: str
    ) -> List[CompletionCandidate]:
                    ...
                    if word_prefix in func["name"].lower() or not word_prefix:
                        completions.append(
                            CompletionCandidate(
                                ...
                                score=(
                                    0.8 if word_prefix in func["name"].lower() else 0.5
                                ),
                            )
                        )
                    ...
                    if word_prefix in cls["name"].lower() or not word_prefix:
                        completions.append(
                            CompletionCandidate(
                                ...
                                score=(
                                    0.8 if word_prefix in cls["name"].lower() else 0.5
                                ),
                            )
                        )
                    ...
                    if word_prefix in var["name"].lower() or not word_prefix:
                        completions.append(
                            CompletionCandidate(
                                ...
                                score=(
                                    0.7 if word_prefix in var["name"].lower() else 0.4
                                ),
                            )
                        )
    ...
    def _get_rag_completions(
        self, context: Dict[str, Any], file_path: str, language: str
    ) -> List[CompletionCandidate]:
            ...
            for pattern in func_patterns:
                ...
                for match in matches:
                    ...
                    if word_prefix in name.lower() or not word_prefix:
                        ...
                        lines = content.split("\n")
                        func_line = next(
                            (i for i, line in enumerate(lines) if name in line), 0
                        )
        ...
        if language == "python":
            ...
            for keyword in keywords:
                if word_prefix in keyword or not word_prefix:
                    completions.append(
                        CompletionCandidate(
                            text=keyword, label=keyword, kind="keyword", score=0.5
                        )
                    )
...
Return only the completion text, one per line, without explanations."""
    ...
    def _deduplicate_completions(
        self, completions: List[CompletionCandidate]
    ) -> List[CompletionCandidate]:
    ...
    def _rank_completions(
        self, completions: List[CompletionCandidate], context: Dict[str, Any]
    ) -> List[CompletionCandidate]:
    ...
    def _get_cache_key(
        self, file_path: str, context: Dict[str, Any], line: int, column: int
    ) -> str:
