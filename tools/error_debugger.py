...
class ErrorDebugger:
    ...
    def _get_error_context(self, parsed_error: ParsedError) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.file_ops.read_file(
                str(file_path), offset=max(1, parsed_error.primary_line - 10), limit=20
            )

            if result.get("exists"):
                lines = result.get("content", "").split("\n")
                error_line_idx = (
                    parsed_error.primary_line
                    - max(1, parsed_error.primary_line - 10)
                    - 1
                )
    ...
    def _generate_suggestions(
        self, parsed_error: ParsedError, relevant_code: List[Dict]
    ) -> List[str]:
        ...
        if "nameerror" in error_type.lower() or "name" in error_msg:
            suggestions.append(
                "Check if the variable/function name is defined and spelled correctly"
            )
        ...
        if "indentationerror" in error_type.lower():
            suggestions.append(
                "Check indentation consistency (use spaces or tabs, not both)"
            )
        ...
        if relevant_code:
            suggestions.append(
                "Review the relevant code chunks retrieved from your codebase"
            )
            suggestions.append(
