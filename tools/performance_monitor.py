...
class PerformanceMonitor:
    ...
    def end_indexing(
        self, files_indexed: int, chunks_created: int, embeddings_generated: int
    ) -> IndexingStats:
        """
    ...
    def record_response_time(
        self, response_time: float, metadata: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        with self._lock:
            ...
            self._record_metric(
                metric_type="response", value=response_time, metadata=metadata
            )
    ...
    def get_current_stats(self) -> Dict[str, Any]:
        ...
        try:
            recent_memory_metrics = [
                m.value for m in recent_metrics if m.metric_type == "memory"
            ]
        ...
        return {
            "memory": {"current_mb": memory_mb, "peak_mb": peak_mb},
            "response_times": response_stats,
            "indexing": asdict(latest_indexing) if latest_indexing else None,
            "system": {"cpu_percent": cpu_percent, "threads": threads},
    ...
    def _save_metrics(self):
        ...
        try:
            ...
            data = {
                ...
                "indexing_stats": [
                    asdict(s) for s in self.indexing_stats[-50:]
                ],  # Last 50 indexing runs
        ...
        try:
            if self.metrics_file.exists():
                ...
                if file_size > 10 * 1024 * 1024:  # Skip if > 10MB
                    print(
                        f"[WARN] Metrics file too large ({file_size / 1024 / 1024:.1f}MB), skipping load"
                    )
                    return
        ...
        with self._lock:
            ...
            for model_id, stats in model_stats.items():
                if stats["responseTimes"]:
                    stats["avgResponseTime"] = sum(stats["responseTimes"]) / len(
                        stats["responseTimes"]
                    )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if (
            ...
            and current_time - self._summary_cache_time < self._summary_cache_ttl
        ):
