...
class RulesEngine:
    ...
    def __init__(self, workspace_path: str = "."):
        ...
        self._file_mtime: float = (
            0.0  # Track file modification time for cache invalidation
        )
...
"""
        ...
        if "Always be helpful" in base_prompt:
            return base_prompt.replace(
                "Always be helpful", f"{rules_section}\nAlways be helpful"
            )
        else:
    ...
    def validate_rules(self, content: Optional[str] = None) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines, 1):
            if len(line) > 500:
                warnings.append(
                    f"Line {i}: Very long line ({len(line)} chars) - consider breaking it up"
                )
