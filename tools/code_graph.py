...
class CodeGraph:
    ...
    call_graph: Dict[str, Set[str]] = field(
        default_factory=dict
...
class CodeGraphBuilder:
    ...
    def get_call_graph(
        ...
        if function_name:
            ...
            callers = {
                src
                for src, targets in self.graph.call_graph.items()
                if function_name in targets
            }
            callees = self.graph.call_graph.get(function_name, set())
            return {"function": function_name, "callers": callers, "callees": callees}
    ...
    def find_related_symbols(
        self, symbol_name: str, relationship_types: List[str] = None
    ) -> List[SymbolNode]:
        """
        ...
        for edge in self.graph.edges:
            if edge.source == symbol_name or edge.target == symbol_name:
                if edge.edge_type in relationship_types:
                    ...
                    related_name = (
                        edge.target if edge.source == symbol_name else edge.source
                    )
                    for node_id, node in self.graph.nodes.items():
    ...
    def get_symbol_dependencies(self, symbol_name: str) -> Dict[str, List[str]]:
        ...
        """
        dependencies = {"calls": [], "imports": [], "inherits": []}
