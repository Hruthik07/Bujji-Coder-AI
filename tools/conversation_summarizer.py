...
def _write_debug_log(location: str, message: str, data: dict, hypothesis_id: str = "A") -> None:
...
class ConversationSummarizer:
    ...
    def __init__(self, llm_provider: Optional[LLMProvider] = None):
        ...
        if not self.provider:
            try:
                ...
                _write_debug_log(
                    "conversation_summarizer.py:28",
                    "Anthropic provider initialization failed, falling back",
                ...
                try:
                    ...
                    _write_debug_log(
                        "conversation_summarizer.py:40",
                        "All LLM providers failed to initialize",
                        {"error_type": type(e2).__name__, "error_message": str(e2)},
                    )
                    self.provider = None
                    self.summary_model = None
    ...
    def summarize_messages(
        ...
        """
        _write_debug_log(
            "conversation_summarizer.py:64",
            "summarize_messages called",
            {"has_provider": self.provider is not None, "message_count": len(messages)},
            hypothesis_id="B",
        )

        if not self.provider:
            _write_debug_log(
                "conversation_summarizer.py:68",
                "No provider available, returning messages as-is",
                {},
                hypothesis_id="B",
            )
            return {
...
Summary:"""
    ...
    def merge_summary(
        self, existing_summary: Optional[str], new_messages: List[Dict[str, str]]
    ) -> str:
        ...
        if not existing_summary:
            ...
            return (
                result["summary_message"]["content"]
