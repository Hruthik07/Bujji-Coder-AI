...
# Supports OpenAI (cloud) and sentence-transformers (free, local).
# Set EMBEDDING_PROVIDER=sentence_transformers to avoid needing OPENAI_API_KEY.
# ---------------------------------------------------------------------------
...
class _SentenceTransformerEmbeddingProvider:
    """Free local embedding provider backed by sentence-transformers."""

    def __init__(self, model_name: str) -> None:
        try:
            from sentence_transformers import SentenceTransformer  # type: ignore
            self._model = SentenceTransformer(model_name)
            self.embedding_dim: int = self._model.get_sentence_embedding_dimension() or 384
...
def _get_embedding_provider(config):
    """Factory: return the configured embedding provider."""
    provider_name = getattr(config, "EMBEDDING_PROVIDER", "openai").lower()
    if provider_name == "sentence_transformers":
        model = getattr(config, "SENTENCE_TRANSFORMERS_MODEL", "all-MiniLM-L6-v2")
        return _SentenceTransformerEmbeddingProvider(model)
    # Default: OpenAI
    ...
    return _OpenAIEmbeddingProvider(config.OPENAI_API_KEY, config.EMBEDDING_MODEL)
...
class RAGSystem:
    ...
    def __init__(
        ...
        # Initialise embedding provider (OpenAI or local sentence-transformers)
        try:
            self._embedding_provider = _get_embedding_provider(Config)
        except Exception as e:
            print(f"[WARN] Embedding provider failed to initialize: {e}")
            print("[WARN] RAG indexing and retrieval will be unavailable.")
    ...
    def index_codebase(self, force_reindex: bool = False) -> Dict[str, Any]:
        ...
        if self._embedding_provider is None:
            return {"status": "error", "message": "Embedding provider not initialized. Check API keys or set EMBEDDING_PROVIDER=sentence_transformers."}
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    if result["success"]:
                        ...
                        print(
                            f"   [WARN] Error indexing {result['file']}: {result.get('error', 'Unknown error')}"
                        )
                    progress.update(task, advance=1)

        elapsed = time.time() - start_time
        print(
            f"[OK] Indexed {indexed_files} files in {elapsed:.2f}s (parallel processing)"
        )
        ...
        for chunk in all_chunks:
            ...
            if estimated_tokens > 8000:
                ...
                if len(sub_chunks) > 1:
                    print(
                        f"[INFO] Split large chunk from {chunk.file_path} ({estimated_tokens} tokens) into {len(sub_chunks)} sub-chunks"
                    )
            else:
        ...
        print(
            f"[INFO] Processed {len(all_chunks)} chunks (after splitting large chunks)"
        )
        ...
        for i in range(0, len(all_chunks), batch_size):
            ...
            for sub_batch_idx, sub_batch_texts in enumerate(sub_batches):
                # Generate embeddings
                try:
                    batch_embeddings = self._embedding_provider.embed_batch(sub_batch_texts)

                    # Validate that embeddings match texts
                    if len(batch_embeddings) != len(sub_batch_texts):
                        print(
                            f"[WARN] Embedding count ({len(batch_embeddings)}) doesn't match text count ({len(sub_batch_texts)}). Skipping batch."
                        )
                        continue
                    ...
                    for text_idx, text in enumerate(sub_batch_texts):
                        if text in text_to_chunk_map:
                            ...
                            print(
                                f"[WARN] Text not found in chunk map at index {text_idx}. Skipping."
                            )
                    ...
                    if len(sub_batch_chunks) != len(batch_embeddings):
                        print(
                            f"[WARN] Chunk count ({len(sub_batch_chunks)}) doesn't match embedding count ({len(batch_embeddings)}). Skipping batch."
                        )
                        continue
                    ...
                    texts.extend(
                        sub_batch_texts[: len(sub_batch_chunks)]
                    )  # Only add texts that have chunks
                    ...
                    print(
                        f"[WARN] Error generating embeddings for batch ({len(sub_batch_texts)} items): {e}"
                    )
    ...
    def retrieve(
        ...
        """
        if not self.is_indexed or self._embedding_provider is None:
            return []
    ...
    def _enhance_with_graph(
        ...
        for result in results[: top_k // 2]:  # Only enhance top results
            ...
            related = self.code_graph.find_related_symbols(
                symbol_name, relationship_types=["calls", "inherits"]
            )
            ...
            for related_node in related[:2]:  # Limit related symbols
                ...
                if related_id not in added_symbols:
                    ...
                    related_chunks = self._semantic_retrieve(
                        related_node.name, top_k=1, file_filter=related_node.file_path
                    )
                    if related_chunks:
    ...
    def _semantic_retrieve(
        self, query: str, top_k: int, file_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _hybrid_retrieve(
        ...
        for chunk in semantic_results:
            ...
            keyword_matches = sum(
                1 for kw in keywords if kw in content_lower or kw in symbol_lower
            )
    ...
    def _rerank_results(
        self, chunks: List[Dict[str, Any]], query: str, top_k: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        for chunk in chunks:
            ...
            if symbol_name:
                ...
                symbol_match = len(query_words & symbol_words) / max(
                    len(query_words), 1
                )
        ...
        if results["documents"] and len(results["documents"][0]) > 0:
            for i in range(len(results["documents"][0])):
                chunk_data = {
                    ...
                    "parent_symbol": results["metadatas"][0][i].get(
                        "parent_symbol", ""
                    ),
                    "distance": (
    ...
    def get_context_for_query(
        ...
        chunks = self.retrieve(
            query,
            top_k=max_chunks or Config.TOP_K_RETRIEVAL,
    ...
    def _chunk_python_file(self, file_path: Path) -> List[CodeChunk]:
        ...
        try:
            ...
            for cls in tree.get("classes", []):
                cls_content = "\n".join(
                    lines[cls["line"] - 1 : cls.get("line_end", cls["line"] + 20)]
                )
                chunks.append(
                    CodeChunk(
                        content=cls_content,
            ...
            for func in tree.get("functions", []):
                func_content = "\n".join(
                    lines[func["line"] - 1 : func.get("line_end", func["line"] + 20)]
                )
                chunks.append(
                    CodeChunk(
    ...
    def _split_large_chunk(self, chunk: CodeChunk) -> List[CodeChunk]:
        ...
        max_lines_per_chunk = max(
            50, max_lines_per_chunk
        )  # At least 50 lines per chunk
        ...
        for i, line in enumerate(lines):
            ...
            if len(current_lines) >= max_lines_per_chunk:
                ...
                sub_chunk = CodeChunk(
                    ...
                    symbol_name=(
                        chunk.symbol_name if i == 0 else None
                    ),  # Only first chunk keeps symbol name
    ...
    def index_file(
        ...
        try:
            ...
            return {"status": "error", "file": file_str, "error": str(e)}
    ...
    def _remove_file_chunks(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self.collection.get(where={"file_path": {"$eq": file_path}})
