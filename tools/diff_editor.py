...
class DiffEditor:
    ...
    def get_file_hash(self, file_path: str) -> Optional[str]:
        """Return MD5 hash of a file's content, or None if the file doesn't exist."""
    ...
    def apply_diff(
        ...
        Args:
            ...
            expected_hash: MD5 hash of the file as seen by the LLM. If provided and
                           the current file has a different hash, the diff is rejected
                           to prevent applying changes to a file that was modified
                           after the LLM read it.
        ...
        try:
            ...
            if file_path.exists():
                ...
                # Conflict detection: reject if file changed since LLM read it
                if expected_hash is not None:
                    actual_hash = hashlib.md5(current_content.encode()).hexdigest()
                    if actual_hash != expected_hash:
                        return {
                            "success": False,
                            "conflict": True,
                            "error": (
                                f"File '{file_path.name}' was modified after the LLM read it "
            ...
            for hunk in file_diff.hunks:
                result = self._apply_hunk(
                    new_lines, hunk, file_diff.old_path and file_path.exists()
                )
    ...
    def apply_diffs(
        self, diffs: List[FileDiff], dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        for file_diff in diffs:
            ...
            if not result["success"]:
                ...
                results["errors"].append(
                    {"file": result.get("file"), "error": result.get("error")}
                )

        return results

    def generate_diff(
        self, old_content: str, new_content: str, file_path: str = "file.txt"
    ) -> str:
        """
    ...
    def preview_diff(self, diff_text: str) -> Dict[str, Any]:
        ...
        preview = {"files_affected": len(file_diffs), "files": []}
    ...
    def validate_diff(
        self, diff_text: str, use_validation_service: bool = True
                ...
                for hunk in file_diff.hunks:
                    ...
                    if file_path.exists():
                        ...
                        if hunk.old_start > len(lines):
                            return (
                                False,
                                f"Hunk start line {hunk.old_start} exceeds file length {len(lines)}",
                                None,
                            )
                        ...
                        if new_content is not None:
                            ...
                            if not validation_result.valid or any(
                                ...
                                if error_messages:
                                    return (
                                        False,
                                        f"Validation errors: {'; '.join(error_messages[:3])}",
                                        validation_results,
                                    )
                    except Exception as e:
    ...
    def _apply_hunk(
        self, lines: List[str], hunk: DiffHunk, file_exists: bool
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result_lines = (
                lines[:start_idx] + new_lines + lines[start_idx + hunk.old_count :]
            )

            return {"success": True, "lines": result_lines}
