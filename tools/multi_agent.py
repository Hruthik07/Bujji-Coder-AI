...
Retrieval and planning run concurrently via asyncio for better performance.
"""
...
class PlanningAgent:
    ...
    def execute(self, task: AgentTask) -> Dict[str, Any]:
        ...
        plan = {"steps": [], "files_affected": [], "dependencies": []}
        ...
        if (
            "modify" in query.lower()
            or "change" in query.lower()
...
class ValidationAgent:
    ...
    def _get_validation_suggestions(self, error: str) -> List[str]:
        ...
        if "line" in error.lower() and "exceeds" in error.lower():
            suggestions.append(
                "Check that the line numbers in the diff match the current file"
            )
        if "syntax" in error.lower():
            suggestions.append(
                "Verify the diff syntax is correct (unified diff format)"
            )
...
class MultiAgentSystem:
    ...
    async def process_request_async(self, query: str) -> Dict[str, Any]:
        """
        Process a complex request using multiple agents running concurrently.
        RetrievalAgent and PlanningAgent execute in parallel via asyncio.gather().
        ...
        """
        results: Dict[str, Any] = {"query": query, "agents": {}}
        ...
        if self.retrieval_agent:
            ...
            tasks.append(loop.run_in_executor(executor, self.retrieval_agent.execute, retrieval_task))
            task_names.append("retriever")

        planning_task = AgentTask(role=AgentRole.PLANNER, query=query, context={})
        tasks.append(loop.run_in_executor(executor, self.planning_agent.execute, planning_task))
        task_names.append("planner")

        agent_results = await asyncio.gather(*tasks, return_exceptions=True)

        for name, result in zip(task_names, agent_results):
            if isinstance(result, Exception):
                results["agents"][name] = {"status": "error", "message": str(result)}
            else:
    ...
    def process_request(self, query: str) -> Dict[str, Any]:
        ...
        Runs agents concurrently when called from an async context,
        ...
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # Already inside an event loop (e.g. FastAPI WebSocket handler):
                # schedule coroutine and wait via thread executor
                import concurrent.futures
                future = asyncio.run_coroutine_threadsafe(self.process_request_async(query), loop)
                return future.result(timeout=30)
            else:
                return loop.run_until_complete(self.process_request_async(query))
        except Exception:
            # Fall back to sequential on any error
            return self._process_request_sequential(query)

    def _process_request_sequential(self, query: str) -> Dict[str, Any]:
        """Sequential fallback for process_request."""
        results: Dict[str, Any] = {"query": query, "agents": {}}

        if self.retrieval_agent:
            retrieval_task = AgentTask(role=AgentRole.RETRIEVER, query=query)
            results["agents"]["retriever"] = self.retrieval_agent.execute(retrieval_task)
        ...
        results["agents"]["planner"] = self.planning_agent.execute(planning_task)
