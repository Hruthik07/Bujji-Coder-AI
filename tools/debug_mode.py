...
def _write_debug_log(location: str, message: str, data: dict, hypothesis_id: str = "C") -> None:
    """Write a debug log entry to the project-relative log file."""
    try:
                ...
                _json_lib.dumps(
                    {
                        ...
                        "message": message,
                        "data": data,
                        "timestamp": int(_time_lib.time() * 1000),
                    }
                )
                + "\n"
            )
...
class DebugMode:
    ...
    def debug_codebase(
        ...
        _write_debug_log("debug_mode.py:52", "Starting codebase scan", {"workspace_path": str(self.scanner.workspace_path)})
        try:
            files = self.scanner.scan_codebase()
            _write_debug_log("debug_mode.py:56", "Scan completed", {"files_found": len(files)})
        except Exception as e:
            _write_debug_log("debug_mode.py:59", "Scan failed", {"error_type": type(e).__name__, "error_message": str(e)})
            files = []
        ...
        if file_pattern:
            ...
            _write_debug_log("debug_mode.py:68", "Pattern filter applied", {"pattern": file_pattern, "before": files_before, "after": len(files)})
        ...
        _write_debug_log("debug_mode.py:76", "Files scanned count set", {"count": len(files)})
        ...
        if auto_fix and all_bugs:
            ...
            results["bugs_fixed"] = [
                {"bug": self._bug_to_dict(item["bug"]), "fix": item["fix"]}
