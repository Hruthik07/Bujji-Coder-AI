...
class IncrementalIndexer:
    ...
    def __init__(self, rag_system: RAGSystem, workspace_path: str = "."):
        ...
        try:
            self.file_watcher = FileWatcher(
                workspace_path=str(self.workspace_path), on_change=self._on_file_changed
            )
        except ImportError:
            print(
                "⚠️  File watcher not available. Install watchdog for incremental indexing."
            )
    ...
    def _index_worker(self):
        ...
        while self.running:
            try:
                ...
                ready_tasks = [
                    (fp, action)
                    for fp, (action, ts) in pending_tasks.items()
                    if current_time >= ts
                ]
    ...
    def _index_file_safe(self, file_path: Path):
        ...
        try:
            ...
            if result.get("status") == "success":
                print(
                    f"✅ Incrementally indexed: {file_path.name} ({result.get('chunks_created', 0)} chunks)"
                )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "watcher_active": (
                self.file_watcher is not None
                and (
                    self.file_watcher.observer is not None
                    and self.file_watcher.observer.is_alive()
                )
                if self.file_watcher
                else False
            ),
