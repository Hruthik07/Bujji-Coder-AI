...
class FileWatcher:
    ...
    def __init__(
        self, workspace_path: str, on_change: Optional[Callable[[Path], None]] = None
    ):
        if not WATCHDOG_AVAILABLE:
            raise ImportError(
                "watchdog is required. Install with: pip install watchdog"
            )
    ...
    def stop(self):
        ...
        if self.observer:
            ...
            if self.observer.is_alive():
                print("[WARN] FileWatcher: observer did not stop cleanly within 5s")
