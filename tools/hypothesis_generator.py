...
class HypothesisGenerator:
    ...
    def generate_hypotheses(
        ...
        prompt = self._build_hypothesis_prompt(
            bug_description, error_text, file_path, code_context, rag_context
        )
...
- Integration problems""",
    ...
    def _build_hypothesis_prompt(
        ...
        parts.append(
            "For each hypothesis, provide description, confidence, reasoning, and suggested instrumentation."
        )
    ...
    def _parse_hypotheses(self, response_text: str) -> List[Hypothesis]:
        ...
        matches = re.finditer(
            hypothesis_pattern, response_text, re.IGNORECASE | re.DOTALL
        )
        ...
        for i, match in enumerate(matches):
            ...
            reasoning_match = re.search(
                r"reasoning[:\s]+(.+?)(?=(?:suggest|instrument|fix|$))",
                content,
            ...
            inst_match = re.search(
                r"(?:instrument|log|debug)[:\s]+(.+?)(?=(?:fix|suggest|$))",
                content,
                re.IGNORECASE | re.DOTALL,
            )
            instrumentation = []
            ...
            fix_match = re.search(
                r"(?:fix|solution)[:\s]+(.+?)(?=(?:hypothesis|theory|$))",
                content,
                re.IGNORECASE | re.DOTALL,
            )
        ...
        if not hypotheses:
            ...
            for line in lines:
                ...
                if re.match(
                    r"^\d+[\.\)]|^[-*]|^(?:hypothesis|theory)", line, re.IGNORECASE
                ):
