...
class TaskClassifier:
    ...
    # Negation words: phrases following these are removed before keyword scoring
    _NEGATION_WORDS = frozenset(
        ["don't", "dont", "do not", "never", "stop", "avoid", "without", "no need to"]
    )
    ...
    @classmethod
    def _remove_negated_spans(cls, message: str) -> str:
        """
        Remove 2-3 words following a negation word so negated keywords don't score.
        Example: "don't create a file" → " a file" (create removed)
        """
        pattern = (
            r"\b("
            + "|".join(re.escape(w) for w in cls._NEGATION_WORDS)
            + r")\b\s+(\w+(?:\s+\w+){0,2})"
        )
    ...
    def classify(
        ...
        # Strip negated spans before scoring so "don't create" doesn't hit code-gen keywords
        cleaned = self._remove_negated_spans(message_lower)

        # Check for explicit code generation intent
        code_gen_score = self._score_code_generation(cleaned)
        complex_score = self._score_complex_task(cleaned)
        ...
        if code_gen_score > complex_score and code_gen_score >= 2:
            ...
            return {
                ...
                "confidence": (
                    min(complex_score / 5.0, 1.0) if complex_score > 0 else 0.5
                ),
            }
    ...
    def _analyze_recent_context(self, recent_messages: List[Dict]) -> Dict[str, float]:
        ...
        for msg in recent_messages:
            ...
            if any(
                word in content
                for word in ["created", "generated", "added", "implemented"]
            ):
                code_gen_bias += 0.5
            ...
            if any(
                word in content
                for word in ["explained", "debugged", "analyzed", "refactored"]
            ):
                complex_bias += 0.5

        return {"code_gen_bias": code_gen_bias, "complex_bias": complex_bias}
